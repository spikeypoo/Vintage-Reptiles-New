...
export async function PUT(request: Request){
  ...
  if (await url4 != "")
    ...
    let isExist = await client.db("Products").collection("Females").find({"_id": new ObjectId(id)}).toArray()
    ...
    if (isExist[0].price != "" && "stripeid" in isExist[0] == false)
    {
      ...
      client.db("Products").collection("Females").findOneAndUpdate({"_id": new ObjectId(id)}, {$set: { "stripeid": res.id }})
      ...
      client.db("Products").collection("Females").findOneAndUpdate({"_id": new ObjectId(id)}, {$set: { "priceid": price.id }})
