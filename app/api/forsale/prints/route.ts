...
export async function GET() {
  ...
  const cursor = await prisma.prints.findMany({select: {id: true, name: true, price: true, image1: true , issale: true, oldprice: true, description: true, stock: true}, orderBy: {id: 'desc'}})
...
export async function PUT(request: Request){
  ...
  if (await url4 != "")
    ...
    client.db("Products").collection("Prints").findOneAndUpdate({"_id": new ObjectId(id)}, {$set: { "name": files.get("name"), "price": files.get("price"), "description": files.get("description"), "issale" : files.get("issale"), "oldprice" : files.get("oldprice"), "stock" : files.get("stock")}})
    let isExist = await client.db("Products").collection("Prints").find({"_id": new ObjectId(id)}).toArray()
    ...
    if (isExist[0].price != "" && "stripeid" in isExist[0] == false)
    {
      ...
      client.db("Products").collection("Prints").findOneAndUpdate({"_id": new ObjectId(id)}, {$set: { "stripeid": res.id }})
      ...
      client.db("Products").collection("Prints").findOneAndUpdate({"_id": new ObjectId(id)}, {$set: { "priceid": price.id }})
