...
export async function GET() {
  ...
  const cursor = await prisma.plants.findMany({select: {id: true, name: true, price: true, image1: true , issale: true, oldprice: true, description: true, stock: true}, orderBy: {id: 'desc'}})
...
export async function PUT(request: Request){
  ...
  if (await url4 != "")
    ...
    client.db("Products").collection("Plants").findOneAndUpdate({"_id": new ObjectId(id)}, {$set: { "name": files.get("name"), "price": files.get("price"), "description": files.get("description"), "issale" : files.get("issale"), "oldprice" : files.get("oldprice"), "stock" : files.get("stock")}})
